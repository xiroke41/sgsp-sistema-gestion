...
function openModal(type, lineId = null, lineName = null) {
  ...
  if (type === 'edit-groups') {
    ...
    document.querySelectorAll('[data-group-editor-operator]').forEach((card) => card.addEventListener('dragstart', (event) => { event.dataTransfer.setData('text/plain', card.dataset.groupEditorOperator); }));
    document.querySelectorAll('[data-group-editor-operator]').forEach((card) => card.addEventListener('click', () => { selectedGroupEditorId = card.dataset.groupEditorOperator; document.querySelectorAll('.rotation-editor-card').forEach((item) => item.classList.remove('is-selected')); card.classList.add('is-selected'); }));
    document.querySelectorAll('[data-editor-position]').forEach((zone) => {
      ...
      zone.addEventListener('drop', (event) => { event.preventDefault(); zone.classList.remove('is-over'); const card = document.querySelector(`[data-group-editor-operator="${event.dataTransfer.getData('text/plain')}"]`); if (card) { zone.querySelector('.rotation-editor-zone-body').appendChild(card); card.dataset.positionGroup = zone.dataset.editorPosition; } });
      zone.addEventListener('click', (event) => { if (!selectedGroupEditorId || event.target.closest('[data-group-editor-operator]')) return; const card = document.querySelector(`[data-group-editor-operator="${selectedGroupEditorId}"]`); if (card) { zone.querySelector('.rotation-editor-zone-body').appendChild(card); card.dataset.positionGroup = zone.dataset.editorPosition; card.classList.remove('is-selected'); selectedGroupEditorId = null; } });
    });
  ...
  if (type === 'edit-colacion-groups') {
    ...
    document.querySelectorAll('[data-colacion-editor-operator]').forEach((card) => card.addEventListener('dragstart', (event) => { event.dataTransfer.setData('text/plain', card.dataset.colacionEditorOperator); }));
    document.querySelectorAll('[data-colacion-editor-operator]').forEach((card) => card.addEventListener('click', () => { selectedColacionEditorId = card.dataset.colacionEditorOperator; document.querySelectorAll('.rotation-editor-card').forEach((item) => item.classList.remove('is-selected')); card.classList.add('is-selected'); }));
    document.querySelectorAll('[data-colacion-zone]').forEach((zone) => {
      zone.addEventListener('dragover', (event) => { event.preventDefault(); zone.classList.add('is-over'); });
      zone.addEventListener('dragleave', () => zone.classList.remove('is-over'));
      zone.addEventListener('drop', (event) => { event.preventDefault(); zone.classList.remove('is-over'); const card = document.querySelector(`[data-colacion-editor-operator="${event.dataTransfer.getData('text/plain')}"]`); if (card) { zone.querySelector('.rotation-editor-zone-body').appendChild(card); card.dataset.colacionGroup = zone.dataset.colacionZone; } });
      zone.addEventListener('click', (event) => { if (!selectedColacionEditorId || event.target.closest('[data-colacion-editor-operator]')) return; const card = document.querySelector(`[data-colacion-editor-operator="${selectedColacionEditorId}"]`); if (card) { zone.querySelector('.rotation-editor-zone-body').appendChild(card); card.dataset.colacionGroup = zone.dataset.colacionZone; card.classList.remove('is-selected'); selectedColacionEditorId = null; } });
    });
